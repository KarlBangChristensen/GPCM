...
proc means data=__prob40 sum; run;

		/* conditional probabilities Pr(x1,x2,...|T) given the score */
		data _condprob;
			set _table;
			%do _sc=0 %to &maxscore; 
				if t=&_sc then _condprob=prob/&&_pr_t_&_sc;
			%end;
		run; 

		data _zeros; do t=0 to &maxscore; _zero=0; output; end; run;

		/* conditional probabilities Pr(x1|T), Pr(x2|T), ... given the score - save as macro variables */
		%do _it=1 %to &_nitems;
			%do _k=0 %to &&_max&_it;
				proc means data=_condprob sum;
					where &&_item&_it=&_k;
					var _condprob;
					class t;
					ods output summary=_condprob&_it._&_k;
				run;
				data _condprob&_it._&_k; 
					merge _condprob&_it._&_k _zeros; 
					_cprob=max(_condprob_Sum,_zero);
				run;
				data _null_; 
					set _condprob&_it._&_k;
					%do _sc=0 %to &maxscore; 
						if t=&_sc then call symput("_pr_&_it._&_k._&_sc",trim(left(_condprob_Sum))); 
					%end;
				run;
			%end;
		%end;
		...
		data _simu0;
			set &data;
			t=sum(of &_item1-&&_item&_nitems);
			keep &_item1-&&_item&_nitems t;
		run;  
		/* loop over simulations NSIMU */
		...
		%do _simu=1 %to &nsimu;
			data _simu&_simu;
				do v=1 to &N;
					t=rantbl(0 %do _sc=0 %to &maxscore; , &&_pr_t_&_sc %end;); output;
				end;
			run; 
			data _simu&_simu;
				set _simu&_simu;
				%do _sc=0 %to &maxscore;
					%do _it=1 %to &_nitems;
						if t=&_sc then _it&_it=rantbl(0 %do _k=0 %to &&_max&_it; ,&&_pr_&_it._&_k._&_sc %end;);
					%end;
				%end;
				output;
			run;
	...
	/* PLOTCAT part (if required) */
	%if %upcase(%left(%trim(&PLOTCAT)))=YES %then %do; 
		%put plotting observed and expected numbers for response categories;
	%end;
	/* PLOTMEAN part (if required) */
	%if %upcase(%left(%trim(&PLOTMEAN)))=YES %then %do; 
		%put plotting observed and expected item means;
	 	%put ---------------------------------------------------------;
	/* end of PLOTMEAN part */
	%end;

	/* probability of (repsonse,score) combinations */
	%do _it=1 %to &_nitems;	%do _h=0 %to &&_max&_it;
		proc means data=_table(where=(&&_item&_it=&_h)) sum; 
			var prob;
			by t; 
			output out=_prob&_it.&_h sum=prob&_it.&_h;
		run;
	%end; %end;
	data _prob;
		merge _margprob %do _it=1 %to &_nitems; %do _h=0 %to &&_max&_it; _prob&_it.&_h %end; %end;;
		by t;
	run;
	%do _it=1 %to &_nitems;	%do _h=0 %to &&_max&_it;
		proc means data=_table(where=(&&_item&_it=&_h)) sum; 
			var count;
			by t; 
			output out=_n&_it.&_h sum=n&_it.&_h;
		run;
	%end; %end;
	data _obs;
		merge %do _it=1 %to &_nitems; %do _h=0 %to &&_max&_it; _n&_it.&_h %end; %end;;
		by t;
	run;

	data _fit; merge _prob _obs; run;
	...
	data _fit;
		set _fit;
		%do _sc=0 %to &maxscore;
			if t=&_sc then n=&&N&_sc;
		%end;
		%do _it=1 %to &_nitems;
			%do _h=0 %to &max; 
				if prob&_it.&_h=. then prob&_it.&_h=0;
				if n&_it.&_h=. then n&_it.&_h=0;
			%end;
		%end;
		
		%do _it=1 %to &_nitems;
			mean&_it=0 %do _h=0 %to &max; +&_h*(n&_it.&_h/n) %end;;
			e_&_it=0 %do _h=0 %to &max; +&_h*(prob&_it.&_h/margprob) %end;;
			e2_&_it=0 %do _h=0 %to &max; +&_h**2*(prob&_it.&_h/margprob) %end;;
			v_&_it=e2_&_it-(e_&_it)**2;
			lower_&_it=e_&_it-1.96*(v_&_it)**.5;
			upper_&_it=e_&_it+1.96*(v_&_it)**.5;
		%end;
	run;
	ods listing;
	%do _it=1 %to &_nitems;
   		title2 justify=center "Observed and expected mean score on item &&_item&_it";
	   	symbol1 v=dot c=black i=none w=3;
   		symbol2 v=none c=blue i=join w=3;
   		symbol3 v=none c=gray i=join l=20 w=3;
   		symbol4 v=none c=gray i=join l=20 w=3;
   		proc gplot data=_fit;
   			plot (mean&_it e_&_it lower_&_it upper_&_it)*t/overlay;
   		run; quit;
	%end; 
   	ods listing close;
